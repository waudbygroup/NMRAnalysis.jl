...
function analyse_diffusion(spec::NMRData{T,2}, coherence=SQ(H1)) where T
    ...
    plot!(p2, spec[roi,1], fill=(0,:dodgerblue), linecolor=:navy, label="integration region")
    plot!(p2, spec[noiseroi,1], fill=(0,:orange), linecolor=:red, label="noise region", legend=:topright)
