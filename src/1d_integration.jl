...
"""
function integrate(filename::String, shift::Number, width, noiseregion=nothing)
    ...
    isnothing(noiseregion) && return I
...
function integrate(filename::String, shifts::Vector, width, noiseregion=nothing)
    map(s -> integrate(filename, s, width, noiseregion), shifts)
end

function integrate(filenames::Vector{String}, shifts, width, noiseregion=nothing)
    map(f -> integrate(f, shift, width, noiseregion), filenames)
end
